...
const StormDataTable: React.FC<StormDataTableProps> = ({ activeStorms, focusedStormId, onFocus, stormColors }) => {
  ...
  // Determine table border color based on focused storm
  const focusedColor = storm && stormColors && stormColors[storm.id] ? stormColors[storm.id] : '#22d3ee';

  return (
    <div
      className="w-full bg-hull/40 rounded-sm border overflow-visible shadow-lg backdrop-blur-md relative flex flex-col transition-all duration-500"
      style={{ borderColor: `${focusedColor}40`, boxShadow: `0 0 20px -10px ${focusedColor}20` }}
    >

      {/* Tabs Header */}
      <div className="flex items-center bg-black/40 rounded-t-sm border-b border-white/5 overflow-x-auto scrollbar-hide">
          ...
          activeStorms.map(s => {
            ...
            return (
              <button
                ...
                className={`
                           px-6 py-3 text-xs font-bold uppercase tracking-wider transition-all border-r border-white/5 whitespace-nowrap min-w-[140px] text-center
                            ${isActive
                    ? 'text-white'
                    : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}
                         `}
                style={{
                  background: isActive ? `linear-gradient(to top, ${identityColor}30, transparent)` : undefined,
                  borderBottom: isActive ? `2px solid ${identityColor}` : '2px solid transparent',
                  color: isActive ? identityColor : undefined,
                  boxShadow: isActive ? `inset 0 -10px 20px -10px ${identityColor}20` : 'none'
                }}        >
                <div className="flex flex-col items-center leading-snug">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-sans font-extrabold" style={{ textShadow: isActive ? `0 0 10px ${identityColor}80` : 'none' }}>{s.name}</span>
                    {peakStatus && <span className={`px-1 rounded-sm text-[9px] font-mono border`} style={{ borderColor: isActive ? identityColor : '#334155', color: isActive ? identityColor : '#64748b', background: isActive ? `${identityColor}10` : 'transparent' }}>{peakStatus}</span>}
                  </div>
                  <div className={`text-[9px] font-mono mt-0.5 ${isActive ? 'opacity-90' : 'opacity-50'}`}>
                    {s.year} // {catLabel}
                  </div>
                </div>
              </button>
      ...
      <div className="overflow-x-auto max-h-[500px] overflow-y-auto custom-scrollbar rounded-b-sm">
        <table className="w-full text-left text-sm text-slate-400">
          <thead className="bg-hull text-[10px] uppercase font-bold tracking-widest sticky top-0 z-10 shadow-sm font-sans" style={{ color: `${focusedColor}90`, borderBottom: `1px solid ${focusedColor}30` }}>
            <tr>
              <HeaderTooltip
                label="Date / Time"
                ...
                return (
                  <tr key={`${point.date}-${point.time}-${idx}`} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span
                        className="inline-flex items-center px-1.5 py-0.5 rounded-sm text-[9px] font-bold border shadow-glow-xs"
                        style={{
                          backgroundColor: 'transparent',
                          color: STORM_STATUS_COLORS[point.status],
                          borderColor: STORM_STATUS_COLORS[point.status]
                        }}
                      >
                        {point.status}
                      </span>
