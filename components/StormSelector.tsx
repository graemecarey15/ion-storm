...
const StormSelector: React.FC<StormSelectorProps> = ({
    ...
    return (
        <>
            {/* Trigger Button - Tech Input Style */}
            <button
                onClick={() => setIsOpen(true)}
                className="relative w-full sm:w-72 lg:w-96 bg-hull/50 border border-white/10 hover:border-cyan-500/50 rounded-sm px-4 py-2 text-left flex items-center shadow-lg group transition-all backdrop-blur-md"
            >
                {/* Tech Corner Accents */}
                <div className="absolute top-0 left-0 w-2 h-2 border-t-2 border-l-2 border-cyan-500/30 group-hover:border-cyan-400 transition-colors"></div>
                <div className="absolute bottom-0 right-0 w-2 h-2 border-b-2 border-r-2 border-cyan-500/30 group-hover:border-cyan-400 transition-colors"></div>

                <svg className="h-4 w-4 text-cyan-500/70 group-hover:text-cyan-400 mr-3 transition-colors" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                <div className="flex-1 truncate font-mono text-sm">
                    {activeCount > 0 ? (
                        <span className="text-cyan-100">
                            <span className="text-cyan-500 mr-2">[{activeCount}]</span>
                            SELECTED
                        </span>
                    ) : (
                        <span className="text-slate-500 uppercase tracking-wider text-xs">Initialize Search...</span>
                    )}
                </div>
                <div className="ml-2 hidden sm:flex items-center gap-1">
                    <kbd className="hidden sm:inline-block px-1.5 py-0.5 text-[10px] font-mono text-cyan-500/50 border border-cyan-500/20 rounded-sm">CMD+K</kbd>
                </div>
            </button>
            ...
            {isOpen && createPortal(
                <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[10vh] px-4">
                    ...
                    <div className="relative w-full max-w-lg bg-hull/95 border border-cyan-500/30 rounded-lg shadow-[0_0_50px_-10px_rgba(6,182,212,0.3)] flex flex-col overflow-hidden animate-fade-in backdrop-blur-xl">

                        {/* Search Bar */}
                        <div className="flex items-center px-4 py-4 bg-black/40 border-b border-white/5">
                            <svg className="h-5 w-5 text-cyan-500 mr-3 animate-pulse" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
                            <input
                                ref={inputRef}
                                type="text"
                                className="flex-1 bg-transparent border-none outline-none text-cyan-50 font-mono placeholder-slate-600 text-base h-8 uppercase tracking-wider"
                                placeholder="ENTER DESIGNATOR..."
                                value={search}
                                onChange={(e) => setSearch(e.target.value)}
                                autoComplete="off"
                            />
                            <button onClick={() => setIsOpen(false)} className="ml-2 text-[10px] text-slate-500 hover:text-white px-2 py-1 border border-white/10 hover:border-white/30 transition-colors uppercase font-mono">ESC</button>
                        </div>
                        ...
                        {/* Footer */}
                        <div className="px-4 py-2 bg-black/60 border-t border-white/5 text-[9px] text-slate-500 flex justify-between font-mono uppercase tracking-wider">
                            <div className="flex gap-3">
                                <span className="text-cyan-500/60">{filtered.length} TARGETS</span>
                                <span className={`${activeCount > 10 ? 'text-cyan-400 font-bold glow-text' : ''}`}>
                                    {activeCount} LOCKED
                                </span>
                            </div>
                            <div className="flex gap-4">
                                <span><span className="text-slate-300">↑↓</span> NAV</span>
                                <span><span className="text-slate-300">←→</span> GROUP</span>
                                <span><span className="text-slate-300">↵</span> ENGAGE</span>
                            </div>
                        </div>
